...
以 80 端口为例，其中最后一列即是进程ID：

```PowerShell
C:\Users\ituknown>netstat -ano | find "80" | find "LISTEN"
...
得到进程ID之后再通过 tasklist 命令查看具体的进程信息，防止错误：

```PowerShell
C:\Users\ituknown>tasklist | find "22412"
svchost.exe                  22412 Services                   0     17,200 K
```

3、确认无误之后使用 taskkill 杀死该进程信息：
